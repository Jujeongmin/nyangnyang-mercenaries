...
**미는 커밋 안의 값**과 대조해 다르면 알린다. 작업본이 아니라 커밋을 보는 이유는 실제로
나가는 것이 커밋이라서다.

**2026-08-30 — 막지 않고 알리기만 한다.** 배포해 보고 두 가지가 드러났다:

1. **배포 대상은 이 파일이 아니라 배포 창의 선택이 정한다.**
   · `[Update existing game]` → 마지막으로 배포한 게임을 갱신한다 (링크·세이브 유지)
   · `[Publish as new game]` → 락에 든 verse 로 **새 게임**을 만든다.
     그 값이 이미 쓰이고 있으면 `verseId … is already in use` 로 막힌다
2. **에디터 컨테이너가 배포할 때마다 락을 제 값으로 되돌린다.** 저장소에서 고쳐도
   다음 배포에 덮인다 — 그때마다 푸시를 막으면 일이 안 된다.

서비스 중인 게임은 <https://verse8.io/R7N5n3y> 이고 그 verse 는 `…-1787733207242` 다
(그 페이지의 iframe 주소로 확인했다. 유저 세이브가 여기 쌓여 있다).
`tools/verse.pin` 은 이제 **그 값**을 담는다 — 저장소에 써 있는 값과 다를 수 있고,
다르면 훅이 경고로 알린다. **배포할 때는 언제나 `[Update existing game]` 을 고른다.**
