...
> ## ⚠️ 2026-08-30 개정 — 이 문서에는 **폐기된 안**이 섞여 있다
>
> 아래 세 가지는 단장 확정으로 **안 만든다**. 옛 절을 읽고 그대로 따라가지 마라.
>
> | 폐기된 것 | 지금 | 어디서 확정 |
> |---|---|---|
> | EP1~EP8 마일스톤 웹툰 (41컷) | **EP0 한 편뿐이다** — 게임 첫 시작의 세계관 설명으로 끝 | "게임 첫 시작했을 때 세계관 설명으로 끝" |
> | 배경 + 인물 합성 (3절) | **컷 하나 = 완성 삽화 한 장** (`ST-EP0-01`~`09`) | "배경, 캐릭터 따로 생성하지말고 같이 만들게 해" |
> | 편성·보스·스킬 레이어 (4.2) | 전부 뺐다. 남은 개인화는 **8컷의 직업별 3종**뿐 | 같은 결정 |
> | 컷 안 닉네임 (EP7) | 화자는 **"냥이단장"** 고정 | "웹툰에서도 닉네임 안뜨고 그냥 냥이단장으로" |
> | 컷에 주인공을 그린다 | **주인공이 나오는 컷은 없다** — 얼굴을 그리면 AI 티가 난다. 풍경과 나레이션으로만 간다 (7컷). 주인공은 마지막 컷의 무기로 대신 등장한다 | "고양이가 나오는 7,8번은 AI느낌이 너무 강해서 안만들었어" |
> | 기록 보관함 (5절 · 7절 5단계) | **안 만든다** — 화가 하나뿐이라 목록이 한 줄이고, 그 한 줄을 위해 설정에 자리를 내줄 이유가 없다 | "기록 보관함 기획 없애고" |
>
> **지금 유효한 것:** 6절 코치마크(구현 완료) · 0절 세 규칙 · 2.1 세계관 ·
> EP0 대본(2.3 첫 항목 — 실제로는 인물 없는 7컷으로 다시 짜였다).
> **에셋 프롬프트의 단일 소스는 `에셋_생성_프롬프트.md` 25절**이다 — 이 문서 8절이 아니다.
> 실제 컷·대사는 `game/public/data/story.json`, 화면은 `game/src/view/story.js` 다.
...
### 2.3 컷 대본 — ⚠️ EP0 만 유효 (실제 9컷은 story.json)
...
## 3. 컷을 어떻게 그리는가 — ⚠️ **폐기** (합성 안 한다. 컷 하나 = 그림 한 장)
...
- ~~**기록 보관함**~~ — ⚠️ **폐기**. 화가 하나뿐이라 목록이 한 줄이다. 다시 읽기는 없다.
...
## 7. 구현 순서 — **완료** (2026-08-30)

| 단계 | 내용 | 상태 |
|---|---|---|
| 1 | 코치마크 오버레이 + `coachSteps` 해석 | **완료** — `#coach`(판 4장 + 링 + 말풍선), 11단계는 `tutorial.json > coachSteps` |
| 2 | 스토리 뷰어 + `story.json` | **완료** — `view/story.js`, 컷 하나 = 그림 한 장 |
| 3 | EP0 7컷 (인물 없음) | **완료** — 그림 7장 생성·반영까지 끝 |
| 4 | EP1 이후 | **안 만든다** (폐기) |
| 5 | ~~기록 보관함~~ | **안 만든다** (폐기) |

**순서는 이렇게 굳었다:** 로딩 → EP0 여섯 컷(세계관) → 직업 선택(마지막 컷의 `needs`) →
마지막 컷(모루 위 무기 · "첫 계약을 받아라") → **코치마크가 제작대를 짚는다.**
스토리가 떠 있는 동안 코치마크는 안 뜬다 (`maybeOnboardHint`).

## 8. 에셋 프롬프트 — ⚠️ **폐기** (에셋_생성_프롬프트.md 25절이 단일 소스)
...
**UI-STORY — 이야기 아이콘** (⚠️ 폐기 — 기록 보관함을 안 만들어 쓸 곳이 없다)
```
