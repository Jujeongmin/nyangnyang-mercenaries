...
//
// ── 지급은 누가 하는가 ────────────────────────────────────
// **여기서는 지급하지 않는다** (2026-09-17). onClose 는 "샀다" 는 신호만
// 넘기고, 받는 쪽(main.js > confirmPurchase)이 서버 영수증을 기다렸다가
// 그것으로 준다. 예전에는 onClose 에서 바로 주고 서버 영수증으로 또 줘서
// 정상 결제가 두 번 지급되는 구조였다. 근거는 서버 훅
// (server.js > $onItemPurchased) 이 남긴 영수증 하나다.
